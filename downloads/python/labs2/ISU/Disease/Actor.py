...
class Actor:
    ...
    #
    def act(self):
        print("Iteration %d: Actor %d" % (self.__itCounter, self.__actorID))
        self.__itCounter += 1
    ...
    def setLocation(self, x, y):
        if (self.__world is None):
            raise RuntimeError("World is null")
        if (x < 0 or x >= self.__world.getWidth()):
            raise ValueError(
                "X coordinate is not located within the world region")
        if (y < 0 or y >= self.__world.getHeight()):
            raise ValueError(
                "Y coordinate is not located within the world region")
