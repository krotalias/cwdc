...
import sys
import math
from _16_sierpinski_arrowhead import Sierpinski_arrowhead_curve as Sierpinski2
from _16_sierpinski_arrowhead import draw as draw2
try:
...
def main(argv=None):
    ...
    def resize(event=None):
        ...
        draw2(canvas, pts2, cntVar.get() == 'ON')
    ...
    def redraw(event=None):
        ...
        vertices = [
            [-1, -1],
            [0, -1 + 2 * math.sin(60 * math.pi / 180.0)],
            [1, -1]
        ]
        def pg(n): return (3**n - 1) // 2
        ndiv = slider.get()
        pts = Sierpinski(vertices[0], vertices[1],
                         vertices[2], ndiv, fillVar.get() == 'ON')
        pts2 = Sierpinski2(ndiv, 2.0)
