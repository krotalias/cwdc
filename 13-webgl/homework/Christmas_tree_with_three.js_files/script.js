 ...
 *
 * <p>Why not, am I right? No specification were given saying that the
 * skyboxes had to be used as the 'environment'.</p>
 *
 * The skybox here is implemented using a shader from
 * {@link https://threejs.org/docs/#api/en/renderers/shaders/ShaderLib shaderlib} plus a
 * {@link https://threejs.org/docs/#api/en/materials/ShaderMaterial shader material}.
 * The drawback is that all presents will use the same texture, which corresponds
 * to the last image set (array) passed as argument.
 *
...
function addPresent(group, size, x, y, z, images, imgpath = path) {
  ...
  // Create a mesh for the present, using the cube shader as the material
...
function init() {
  ...
  imageNames.img3 = imageNames.img3.map((img) => "textures/cube/pisa/" + img);
  addPresent(group, 40, 20, -115, -130, imageNames.img3, "");
