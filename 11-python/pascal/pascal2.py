...
#
#   Usage:
#   - \ref /cwdc/11-python/pascal/pascal2.py?0+10
#
#   @author Paulo Roma
#   @since 26/08/2018
#   @see http://en.wikipedia.org/wiki/Pascal_triangle
#   @see <a href="/cwdc/11-python/pascal/pascal2.py?0+10">link</a> (10+10)
#   @see <a href="/cwdc/11-python/showCodePython.php?f=pascal/pascal2">source</a>
...
class Pascal(object):
    ...
    #  @return @f$C{{n}\choose{m}} = {{n!} \over {(n-m)!\ m!}} = {{n (n-1) ... (n-m+1)} \over {m!}}@f$,
    #          number of ways of choosing m objects from n.
    ...
    #   @param ndig  number of digits for right justification to give a triangle shape.
    #   @return string representing the row.
    #
    def fmtRow(self, r, pad=True, ndig=None):
        # adds some blanks for getting the triangle shape
        txt = ' ' * (self.last_level - len(r) + 2) if pad else ''
        if ndig is None:
            txt += ' '.join(map(str, r))                             # show it
        else:
            txt += ' '.join(map(lambda s: str(s).rjust(ndig), r))    # show it
        return txt.rstrip()
    ...
    def __repr__(self):
        ...
        lrow = ba[self.last_level]
        # maximum number of digits
        ndig = len(str(max(ba[self.last_level])))
        # length of the last row
        maxlen = len(lrow) * (ndig + 1)
        ...
        for l in range(self.last_level + 1):
            ...
            if l >= self.first_level:
                if self.center:
                    st += self.fmtRow(row, False,
                                      ndig).center(maxlen) + self.lineFeed
                else:
...
#
def main(argv=None):
    ## Display some information extracted from a Pascal triangle.
    #
    #  @param newline line feed character.
    #
    def displayPascal(newline):
        print("<pre><code>")
        pascal = Pascal(first_level, int(last_level), newline)
        print("This is str(pascal):\n%s%s" % (newline, pascal))
        ...
        print("\nProbability of getting at most %d heads in %d flips is: %f%s" % (
    ...
    if HTML:
        ...
        try:
            ...
            print('Please enter a whole number <br>')
            first_level = 0
            last_level = 12
        ...
        print("""
            <h1><a href="/python/laboratorios.html#pascal">Pascal Triangle</a></h1>
            <p> {} </p>
        """.format(message))
        ...
        print("""
            <form method="get">
                <label for="flevel">First (Heads)</label>
                <input type="number" name="flevel" id="flevel" min=0 max=99 value = {}>
                <label for="llevel">Last (Flips)</label>
                <input type="number" name="llevel" id="llevel"min=0 max=99 value = {} autofocus>
                <input type="submit" value="Pascalize!">
            </form>
        """.format(flevel, llevel))
