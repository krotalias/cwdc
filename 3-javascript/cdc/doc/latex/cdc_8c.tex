...
Nota\+: Achar a taxa \"{}t\"{} que produz o preço à vista \"{}y\"{} requer o método de Newton\+:  \begin{eqnarray*}
    x_{n+1} &=& x_n - \frac{f(x_n)}{f\textnormal{\textquotesingle}(x_n)} \\
      ...
      f\textnormal{\textquotesingle}(t) &=& y (b + t (p-1) a) - xb
    \end{eqnarray*} onde $a = (1+t)^{(p-2)}, b = (1+t)^{(p-1)}, c = (1+t)^{p}$ e o problema é equivalente a encontrar um zero da função f \[t_{n+1} = t_n - \frac{f(t)}{f\textnormal{\textquotesingle}(t)}, t_o = \frac{x}{y}\]
